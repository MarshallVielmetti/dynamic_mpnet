"""
This file was borrowed from https://github.com/FelicienC/RRT-Dubins/blob/master/code/dubins.py
"""
...
class Dubins:
    ...
    def dubins_path(self, start, end) -> tuple:
        """
        ...
        A tuple containing:
              ...
              - The length of the shortest dubins path between the two points (float)
        """
        options = self.all_options(start, end)
        path_length, dubins_path, straight = min(options, key=lambda x: x[0])
        return self.generate_points(start, end, dubins_path, straight), path_length

    def dubins_path_length(self, start, end) -> float:
        """
        Returns the length of the shortest dubins path between two points
        Avoids sampling points along the path.
